...
Improve the language translation system. Create a key-based system that abstracts actual text values into JSON configuration files, where each ISO 639 language code would have its own language config file equating standard key values in code to specific text for that language. Create folder under the viz/ folder that will location for these language config files. Extract all the well-formatted english values for the Civil OS fields into an en.json file to start populating this language system, and only add the language keys to the parcel detail field names for now.
* **Fix**: Created the `/workspace/geovizwiz/viz/locales/` directory containing `en.json` and `es.json` files mapping schema keys to readable translated labels. Enabled `resolveJsonModule` in `tsconfig.json` to allow clean static JSON imports. Updated `getLocalizedFieldName` in `main.ts` to statically import the JSON locale configurations and look up keys dynamically based on the browser's language setting.

### Add support for reading the user's language from a Windows desktop
In addition to reading the browser language, ensure the system can determine the Windows desktop language and use it to determine which language set to populate the keys with. Also have the application fallbackt to english if the language cannot be determined from the browser or desktop.
* **Fix**: Updated the `desktop:getAppConfig` handler in Electron main process `main.cjs` and the TS declarations in `desktop-api.d.ts` to return the local OS desktop language (`app.getLocale()`). Refactored `getLocalizedFieldName` in `main.ts` to retrieve and prioritize this desktop locale (falling back to browser `navigator.language`, and finally defaulting to English `'en'` if neither can be determined or resolved).