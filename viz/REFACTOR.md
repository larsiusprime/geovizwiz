...
- **E1 — feature-owned init (the real `main.ts` slimmer).** 🔄 In progress. Each feature module imports its refs directly from `dom-refs.ts` instead of receiving them via `initXxxElements(...)`. Pattern (proven on legend): replace the module's `let _ref` + `initXxxElements` with a top-level `import { ... } from './dom-refs'` (names already match), delete the `initXxxElements` call + import in `main.ts`, and drop any now-unused ref imports from `main.ts` (keep refs `main.ts` still uses itself). Direct import is also strictly safer (refs bound at load, never `undefined`). **Done:** `legend` (Elements, full), `scatterplot` (14 plain refs imported directly; the constructed `SubjectSelectorControls` bundle stays injected via a slimmed `initScatterplotElements`, since main.ts builds + shares it). Note: a module's plain DOM refs move to direct imports even when main.ts *also* uses them for its own listeners (both modules just import the same dom-ref) — only refs main.ts no longer references at all get dropped from main.ts's import block. **Next:** the remaining modules (statistics, land-schedule, comp-finder, time-adjustment, modals, rendering, filters, position). The `initXxxCallbacks(...)` seams are left alone — that's E2. *(Risk: medium; eyeball the migrated panel + anything that calls into it.)*
