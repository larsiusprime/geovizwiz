...
Done: G1, C3a, G2 (full tsc burndown 103 → 0, gate strict at 0), R1 (colorMode teardown), E1 (feature-owned init — all modules; main.ts no longer threads element objects). Next: optionally **D1** (`S.ui` namespacing) and **E2** (replace the ~134 `initXxxCallbacks` injection seams with an event bus or direct imports — highest risk, only if the coupling becomes painful). Treat D2–D4 and the deferred C items as opt-in.
