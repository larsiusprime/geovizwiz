...
**Deferred (init bodies do real side effects — event binding, widget creation, render calls — so a mechanical ref swap isn't safe; need the bigger self-wire treatment):** `filters`, `time-adjustment`, `comp-finder`. `selection`'s `initSelectionElements()` takes no refs (it creates elements + binds document listeners) — nothing to convert. The `initXxxCallbacks(...)` seams are left alone — that's E2. *(Risk: medium; eyeball the migrated panel + anything that calls into it.)*
